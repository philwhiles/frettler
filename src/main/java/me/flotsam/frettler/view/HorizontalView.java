...
/**
 * Renders chords or scales in a horizontal fretboard representation, where
 * notes appear in the middle of strings, and the 'fretboard' has inlay markers
...
public class HorizontalView implements View {
		...
		for (int i : getOrderedStrings(instrument, false)) {
			...
			for (Fret fret : tonesInString) {
				...
				if (!(instrument.isBanjo() && (fret.getFretNum() > 0 && fret.getFretNum() < 6)
						&& fret.getStringNum() == 0)) {
					note = scaleNotes.stream().filter(sn -> sn.getNote().getPitch() == fret.getNote().getPitch())
							.findFirst();
				}
				...
				if (note.isPresent()) {
					...
					if (options.isIntervals()) {
						fretStr = note.get().getInterval().isPresent() ? note.get().getInterval().get().getLabel()
	...
	private void renderFretPresentRight(int firstFret, StringBuilder stringBuilder, String fretStr, Options options,
			Fret fret) {
	...
	private void renderFretPresentLeft(int firstFret, StringBuilder stringBuilder, String fretStr, Options options,
			Fret fret) {
		Note note = fret.getNote();
		if (options.isColour()) {
			Colour col = options.isOctaves() ? ColourMap.get((Integer) fret.getOctave())
	...
	private String createFretboardSide(boolean upper, Options options) {
		...
		if (options.isLefty()) {
			...
			sb.append(upper ? upperTopL : lowerTopL);
			for (int fret = instrument.getFrets(); fret >= 1; fret--) {
			...
			sb.append(upper ? upperTopR : lowerTopR);
